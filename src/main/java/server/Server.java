...
public class Server {
    ...
    private GameInfo getAllOtherPlayersFoldedInfo() {
        GameInfo gameInfo = new GameInfo("main.server.Server", "main.server.Server");
    ...
    private boolean everyClientHandlerHasAPlayer() {
        List<String> playerIDs = getAllPlayerIDs();
    ...
    private GameInfo getNewRoundStateInfo() {
        ...
        if (serverGame.getRoundState().equals(RoundState.SHOWDOWN)) {
            ...
            gameInfo.setLosingPlayers(serverGame.getPlayersWithNoChips());
            String winningHand = "CODE IS WIP"; //HandEvaluator.getHandName(serverGame.getMainGame().getHighestScore());
            gameInfo.setNameOfWinningHand(winningHand);
            HashMap<Player, List<Card>> playerHands = new HashMap<>();
            for (Player player: serverGame.getPlayers()) {
    ...
    private void gameOver() {
        // Declare winner
        // Show main.model.game over screen
        // close main.server
    }
