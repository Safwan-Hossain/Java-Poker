...
\subsubsection{ model.GameInfo Module (\mref{mModel})}
    ...
    \item[Implemented By:] model.Client, controller.ClientController, handler.ClientHandler
    \end{description}

\subsubsection{ model.Server Module (\mref{mModel})}
    \begin{description}
...
\subsubsection{ model.Game Module (\mref{mModel})}
    \begin{description}
...
\subsubsection{ model.Card Module (\mref{mModel})}
    ...
    \item[Implemented By:] model.Game, model.Deck
    \end{description}
...
\subsubsection{ model.Player Module (\mref{mModel})}
    ...
    \item[Implemented By:] model.Game
    \end{description}
...
\subsubsection{ enumeration.PlayerAction Module (\mref{mModel})}
    \begin{description}
    ...
    \item[Implemented By:] model.Game, model.GameInfo, handler.ClientHandler, model.Client
    \end{description}
% ========== VIEW ========== %
...
\subsubsection{ handler.ClientHandler Module (\mref{mController})}
    \begin{description}
...
\textbf{Routine name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
\hline
new model.Game & model.Player [ ], $\mathbb{I}$ & &\\
\hline
startGame & $\mathbb{I}$ & &\\
\hline 
...
\noindent new model.Game(p\verb|_|list, x):
\begin{itemize}
\item transition:
    \begin{itemize}
...
\section* {controller.MainController Module}
    \subsection* {Uses}
        model.Client, model.Game, Gameview, view.MainMenuView, model.Server
    \subsection* {Syntax}
    
        \subsubsection* {Imported Constants}
            None
        \subsubsection* {Imported Types}
            model.Client, model.Game, model.Server
        \subsubsection* {Exported Access Programs}
...
\section* {controller.ClientController Module}
    \subsection* {Uses}
        model.Client, model.Game, Gameview, enumeration.PlayerAction, model.GameInfo
    \subsection* {Syntax}
    
        \subsubsection* {Imported Constants}
            None
        \subsubsection* {Imported Types}
            model.Client, model.Game, enumeration.PlayerAction, model.GameInfo
        \subsubsection* {Exported Access Programs}
        ...
        \begin{tabular}{| l | l | l | p{4cm} |}
            ...
            \hline 
            getValidPlayerAction & Scanner & enumeration.PlayerAction &\\
            \hline 
            getValidBet & Scanner & $\mathbb{Z}$ &\\
            \hline 
            CreateGameInfo & enumeration.PlayerAction, $\mathbb{Z}$ & model.GameInfo &\\
            \hline 
        \end{tabular}
    ...
    \subsubsection* {Access Routine Semantics}
        ...
        \begin{itemize}
        \item return : $playerAction : enumeration.PlayerAction$
        \item description: Asks the user for a valid player action. If the user input matches a enumeration.PlayerAction enumerator then return the enumeration.PlayerAction enmerator. Otherwise ask again.
        \end{itemize}
...
\section* {handler.ClientHandler ADT Module}
    ...
    \subsection* {Syntax}
        ...
        \begin{tabular}{| l | l | l | p{6cm} |}
            ...
            \textbf{Routine name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
            \hline
            new handler.ClientHandler & Socket & handler.ClientHandler & \\
            \hline
            run &  &  & \\
            \hline 
    ...
    \subsubsection* {State Variables}
        ...
        $\mathit{clientHandlers}:$ static sequence of $handler.ClientHandler$\\
        $\mathit{game}:$ static $model.Game$\\
        $\mathit{socket}: Socket$\\
        $\mathit{inputStream}: ObjectInputStream$\\
        $\mathit{outputStream} : ObjectOutputStream$\\
    ...
    \subsubsection* {Access Routine Semantics}
        ...
        \noindent new handler.ClientHandler(socket):
        \begin{itemize}
        \item return : $self$
        ...
        \item description : Each handler.ClientHandler is responsible for taking in input from a single client connected to a server. Everytime a client sends a command (their game move) to the server, their designated handler.ClientHandler will receive that command and input that command into the game on the server on behalf of the client's name (as if the client had inputted the command directly to the game). Then the handler.ClientHandler will forward the resulting state of the game after the input, synchronizing the game for all clients.
        \end{itemize}
...
\section* {model.Server ADT Module}
    \subsection* {Uses}
    handler.ClientHandler
    \subsection* {Syntax}
    ...
    \subsubsection* {Access Routine Semantics}
        ...
        \noindent new model.Server(serverSocket):
        \begin{itemize}
        \item return : $self$
        ...
        \noindent startServer():
        \begin{itemize}
        \item transition : Listen for any attempts to connect to $serverSocket$ by a model.Client. If an attempt is made, try to get the client's socket and create a new handler.ClientHandler (using the client's socket) on a new thread and start the thread.
        \item description : The handler.ClientHandler is responsible for taking in input from a single model.Client connected to a server. Everytime a model.Client connects to the server, a new handler.ClientHandler will be created on a new Thread to listen for input from that specific model.Client.
        \end{itemize}
...
\section* {model.GameInfo ADT Module}
    ...
    \subsection* {Syntax}
        ...
        \begin{tabular}{| l | l | l | p{6cm} |}
            ...
            \textbf{Routine name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
            \hline
            new model.GameInfo & String, String, enumeration.PlayerAction, $\mathbb{Z}$ & model.GameInfo & \\
            \hline
            getClientID &  & String & \\
            \hline
    ...
    \subsubsection* {State Variables}
        $\mathit{clientID}: String$\\
        $\mathit{playerName}: String$\\
        $\mathit{playerAction}: enumeration.PlayerAction$\\
    ...
    \subsubsection* {Access Routine Semantics}
        ...
        \item description : model.GameInfo is essentially a data structure that model.Client and model.Server will use to communicate.
        \end{itemize}
...
\section* {enumeration.PlayerAction Module}
    \subsection* {Uses}
    enumeration.PlayerAction
    \subsection* {Syntax}
        ...
        \begin{tabular}{| l | l | l | p{6cm} |}
            ...
            \textbf{Routine name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
            \hline
            isABet & enumeration.PlayerAction & $\mathbb{B}$ & \\
            \hline
            actionIsValid & String & $\mathbb{B}$ & \\
            \hline
            getActionByString & String & enumeration.PlayerAction &  IllegalArgumentException\\
            \hline 
        \end{tabular}
    ...
    \subsubsection* {Considerations}
        enumeration.PlayerAction is an enum class that represent the possible actions a player can make
    ...
    \subsubsection* {Access Routine Semantics}
        ...
        \begin{itemize}
        \item return : True if the String $action$ is a enumeration.PlayerAction. False if not.
        \end{itemize}
        
        \noindent getActionByString(action):
        \begin{itemize}
        \item return : Corresponding enumeration.PlayerAction that matches $action$
        \item exception : Throw IllegalArgumentException if there is no  string value for enumeration.PlayerAction that matches $action$
        \end{itemize}
